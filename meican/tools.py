...
class RestUrl(object):
    ...
    def calender_items(cls, detail=False):
        ...
        #temp = cls.get_base_url("preorder/api/v2.1/calendarItems/list", data)
        #return temp
...
class MeiCan(object):
    ...
    def load_tabs(self, refresh=False):
        if not self._calendar_items or refresh:
            ...
            self._data_list = self._calendar_items['dateList'][0]
            self._time_list = self.default_alarm_time()
    ...
    def default_alarm_time(self):
        time_list = []
        for tar in self._data_list['calendarItemList']:
            time_str = tar['openingTime']['defaultAlarmTime']
            temp = str.split(time_str, ':')
            temp_seconds = datetime.timedelta(hours=int(temp[0]), minutes=int(temp[1])).total_seconds()
            time_list.append(temp_seconds)
        return time_list

    def get_price_limit(self, index):
        data = self._data_list['calendarItemList'][index]
        if not data or data['status'] != TabStatus.AVAIL :
            raise NoOrderAvailable("Currently no available orders")
        return data.userTab.corp.priceLimit
